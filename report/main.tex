...
ControlNet \citep{zhang-etal-2023-controlnet} introduces spatial conditioning for pretrained text-to-image diffusion models. Our baseline instantiates this approach using SD1.5, a line-art condition, and an existing line-art LoRA. The final system instead uses FLUX.2-klein-4B, whose official model card describes generation and reference-image editing within one model \citep{flux2-klein-4b-model-card}. We apply editing instructions directly to the source portrait without training a new drawing model. Informative Drawings and CLIPasso are background methods, not additional quantitative baselines; our quantitative comparison is between the configured SD1.5 and FLUX systems.
...
Before assembly, every requested person must have a validated biography and an existing drawing. Otherwise, the current run is marked incomplete and existing PDFs are not rebuilt. ReportLab creates individual and combined A4 PDFs with a title, proportionally fitted drawing, wrapped biography, source link, and available portrait attribution. It refuses biographies that overflow the text area rather than silently truncating them. Individual files are replaced through temporary outputs only after successful rendering.
...
The software entry point is \nolinkurl{main.py}, with a Colab notebook for cloud execution. Appendix~\ref{app:reproducibility} records configurations, prompt templates, commands, artifact locations, and version limits. Appendix~\ref{app:examples} presents all eight unchanged image pairs and the first generated book page; these supplements document the prototype, not additional experiments.
