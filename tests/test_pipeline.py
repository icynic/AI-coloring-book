...
class PipelineHelpersTest(unittest.TestCase):
    ...
    def test_full_colab_notebook_has_valid_code_and_no_repair_cells(self):
        ...
        inspection = "".join(notebook["cells"][8]["source"])
        ...
        self.assertNotIn("FileLink", inspection)
        self.assertNotIn("localhost", inspection)
        self.assertIn("from google.colab import files", inspection)
        self.assertIn("files.download(str(book_path))", inspection)
        self.assertIn("if not book_path.is_file()", inspection)
